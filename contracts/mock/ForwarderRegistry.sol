...
contract ForwarderRegistry is UsingAppendedCallData, IERC2771 {
    ...
    bytes32 internal constant EIP712DOMAIN_NAME =
        keccak256("ForwarderRegistry");
    ...
    function forward(address target, bytes calldata data) external payable {
        ...
        require(
            _forwarders[signer][msg.sender].approved,
            "NOT_AUTHORIZED_FORWARDER"
        );
    ...
    function getNonce(address signer, address forwarder)
        external
        view
        returns (uint256)
    {
    ...
    function isForwarderFor(address signer, address forwarder)
        external
        view
        returns (bool)
    {
        return
            ...
            _forwarders[signer][forwarder].approved;
    }
    ...
    function _encodeMessage(
        ...
        return
            abi.encodePacked(
                ...
                _DOMAIN_SEPARATOR(),
                keccak256(
                    abi.encode(APPROVAL_TYPEHASH, forwarder, approved, nonce)
                )
            );
    }
    ...
    function _requireValidSignature(
        ...
        if (signatureType == SignatureType.EIP1271) {
            require(
                ERC1271(signer).isValidSignature(dataToHash, signature) ==
                    ERC1271_MAGICVALUE,
                "SIGNATURE_1271_INVALID"
            );
        } else if (signatureType == SignatureType.EIP1654) {
            require(
                ERC1654(signer).isValidSignature(
    ...
    function _approveForwarder(
        ...
        Forwarder storage forwarderData = _forwarders[signer][
            forwarderToChangeApproval
        ];
