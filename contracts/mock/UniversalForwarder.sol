...
contract UniversalForwarder is UsingAppendedCallData, IERC2771 {
    ...
    bytes32 internal constant EIP712DOMAIN_NAME =
        keccak256("UniversalForwarder");
    bytes32 internal constant APPROVAL_TYPEHASH =
        keccak256("ApproveForwarderForever(address forwarder)");
    ...
    function _encodeMessage(address forwarder)
        internal
        view
        returns (bytes memory)
    {
        return
            abi.encodePacked(
                "\x19\x01",
                _DOMAIN_SEPARATOR(),
                keccak256(abi.encode(APPROVAL_TYPEHASH, forwarder))
            );
    }
